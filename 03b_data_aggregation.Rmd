# Data Agrregation
## Creating aggregated R Objects {#r-objects}
...
## Creating Phyloseq Objects {#physeq-objects}
### Prerequisite
...
### Phyloseq object using `mothur` output {#mothur-physeq}
```{r}
...
### Phyloseq object using `qiime2` output {#qiime2-physeq}
```{r}
...
### Phyloseq object using `metaphlan` output {#metaphlan-physeq}
```{r}
...
Summarizing `metaphlan` phyloseq object
```{r message=FALSE, warning=FALSE}
microbiome::summarize_phyloseq(ps_m3_raw)
```

## Preparing visualization input data
```{r taxon_rel_abund_data, animation.hook='gifski', interval=4, fig.width=4}
set.seed(110912)
...
mo_taxon_rel_abund <- mo_Rjoined_object %>% 
  ...
  mutate(taxon = str_replace(taxon,
         ...
         taxon = str_replace(taxon,
                             "^(\\S*)$", "*\\1*"))
save(mo_taxon_rel_abund, file = "RDataRDS/taxon_rel_abund.RData")

q2_taxon_rel_abund <- q2_Rjoined_object %>% 
  rename(group = isolate) %>% 
  filter(level == "phylum") %>% 
  group_by(sample_id, taxon, group) %>% 
  summarise(rel_abund = sum(rel_abund), .groups = "drop") %>% 
  ...
  mutate(taxon = str_replace(taxon,
         ...
         taxon = str_replace(taxon,
                             "^(\\S*)$", "*\\1*"))
resave(q2_taxon_rel_abund, file = "RDataRDS/taxon_rel_abund.RData")

m3_taxon_rel_abund1 <- m3_rel_Rjoined_object %>% 
  ...
  mutate(taxon = str_replace(taxon,
         ...
         taxon = str_replace(taxon,
                             "^(\\S*)$", "*\\1*"))
resave(m3_taxon_rel_abund1, file = "RDataRDS/taxon_rel_abund.RData")

m3_taxon_rel_abund2 <- m3_read_count_Rjoined_object %>% 
  rename(group = platform) %>% 
  filter(level == "phylum") %>% 
  group_by(sample_id, taxon, group) %>% 
  summarise(rel_abund = sum(rel_abund), .groups = "drop") %>% 
  ...
  mutate(taxon = str_replace(taxon,
         ...
         taxon = str_replace(taxon,
                             "^(\\S*)$", "*\\1*"))
resave(m3_taxon_rel_abund2, file = "RDataRDS/taxon_rel_abund.RData")
```
