...
mo_glued_taxonomy <- mo_taxonomy %>% 
  mutate(pretty_otu = str_replace(string = otu,
        ...
        genus_otu = glue("{genus}<br>({pretty_otu})"))

inner_join( mo_glued_taxonomy, mo_taxonomy, by = "otu")
  
mo_composite <- inner_join(mo_metadata, mo_otu_counts, by = "sample_id") %>% 
  inner_join( ., mo_glued_taxonomy, by = "otu") %>% 
  group_by(sample_id) %>% 
...
mo_Rjoined_object <- mo_composite %>% 
  pivot_longer(c(all_of(taxlevels), "otu", genus_otu), names_to = "level", values_to = "taxon") %>% 
