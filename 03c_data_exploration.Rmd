...
# Exploring Microbiome Data
- This is the point where the joined objects become handy.
- We will explore joined data saved as `phyloseq` object.
- We will be exploring the microbiome data at Genus level.
...
Let's use object created from `mothur` output
```{r}
ps <- ps_mo_raw
```
...
### Using `geom_col()` from `ggplot2` package
